...
export const ParticleField: React.FC<Props> = ({
  ...
  const particles: Particle[] = Array.from({ length: safeCount }, (_, idx) => {
    ...
    return {
      ...
      color: colorSecondary ? (Math.random() > 0.5 ? color : colorSecondary) : color,
    };
  });
  ...
  return (
    <canvas
      ref={(el) => { if (!el) return; const ctx = el.getContext("2d"); if (ctx) draw(ctx); }}
      width={1920}
      height={1080}
      style={{ position: "absolute", inset: 0, zIndex: layer, pointerEvents: "none" }}
    />
  );
};

export const particleFieldMeta: ComponentMeta = {
