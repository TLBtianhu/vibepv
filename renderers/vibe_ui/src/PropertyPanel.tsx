...
export const PropertyPanel: React.FC<Props> = ({ effectId }) => {
    ...
    if (typeof defaultValue === "number") {
      ...
      return (
        <div key={key} className="mb-4">
          <label className="text-gray-300 text-xs mb-1 block">{key} ({num.toFixed(2)})</label>
      ...
      return (
        <div key={key} className="mb-4">
          ...
          <div className="flex gap-2">
            <input type={isColor ? "color" : "text"} value={str}
              onChange={(e) => updateParam(effectId, key, e.target.value)}
              className={isColor ? "w-10 h-8 rounded cursor-pointer border border-gray-600 bg-gray-900"
  ...
  return (
    <div>
      <h3 className="text-white font-bold mb-4 text-sm">{rule.component || "未知零件"}</h3>
      <p className="text-gray-500 text-xs mb-4">{meta.allowedParams.length} 个可调参数</p>
